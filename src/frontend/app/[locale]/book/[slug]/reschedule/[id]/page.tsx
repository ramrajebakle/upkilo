    ...
    if (error) return (
        <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
            <Card className="max-w-md w-full p-8 text-center space-y-4 shadow-2xl border-none">
                ...
                <h1 className="text-xl font-bold text-slate-900">Oops! {error}</h1>
                <p className="text-slate-500 text-sm">We couldn't find the booking you're looking for. Please check the link in your email.</p>
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col">
            <header className="bg-white border-b border-slate-100 sticky top-0 z-10 shadow-sm">
                <div className="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <Link href={`/book/${slug}`} className="p-2 hover:bg-slate-50 rounded-full transition-colors text-slate-400">
            ...
            <main className="flex-1 max-w-2xl mx-auto w-full px-4 pt-8 md:pt-16 pb-12 space-y-8">
                    ...
                    <Card className="p-10 shadow-2xl shadow-slate-200/50 border-none space-y-8 animate-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="space-y-6 max-w-xs mx-auto">
                            <Input 
                                ...
                                className="text-center font-black tracking-[0.2em] text-2xl h-14 border-2 border-slate-100 focus:border-primary focus:ring-primary/5 rounded-xl transition-all"
                            />
                            <Button onClick={handleVerifyCode} className="w-full font-bold h-12 shadow-xl shadow-primary/25 rounded-xl text-md">
                                Verify Appointment
                            </Button>
                        </div>
                    </Card>
                        ...
                        <Card className="p-0 overflow-hidden shadow-2xl shadow-slate-200/50 border-none">
                            <div className="p-8 border-b border-slate-50 space-y-6">
                                <div className="flex items-start justify-between gap-4">
                                    <div>
                                        <h3 className="text-2xl font-black text-slate-900 mb-1">{booking.service}</h3>
                                        <p className="text-slate-500 font-medium flex items-center gap-1.5">
                                            with <span className="text-slate-800 font-bold">{booking.staff}</span>
                                        </p>
                                    </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-slate-50 rounded-2xl p-6">
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex flex-col">
                                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Current Date</span>
                                                <span className="text-sm font-bold text-slate-700">{new Date(booking.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 rounded-lg bg-white shadow-sm flex items-center justify-center text-primary">
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-sm font-bold text-slate-700 truncate">{booking.address || booking.location}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="px-8 py-6 bg-white flex flex-col sm:flex-row items-center justify-between gap-6">
                                <div className="flex items-center gap-2">
                                    <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                                    <p className="text-xs text-slate-500 font-bold">New slots are currently available</p>
                                </div>
                                <Button 
                ...
                <p className="text-center text-[11px] text-slate-400 max-w-sm mx-auto font-medium leading-relaxed">
