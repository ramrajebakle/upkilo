        ...
        <div className="h-[calc(100vh-8rem)] flex flex-col gap-0">
            ...
            <div className="flex items-center justify-between mb-4 px-1 shrink-0">
                <div>
                    ...
                    <p className="text-foreground-secondary text-sm mt-0.5">Design and preview responsive email templates with live rendering</p>
                </div>
                <div className="flex items-center gap-2">
                    <button
                        onClick={handleSendTest}
                        disabled={!selectedTemplate}
                        className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-foreground-secondary bg-card border border-border rounded-lg hover:bg-accent disabled:opacity-40"
                    >
            ...
            <div className="flex-1 flex gap-4 min-h-0">
                {/* Sidebar */}
                <div className="w-60 shrink-0 bg-card rounded-xl border border-border flex flex-col overflow-hidden">
                            ...
                            <button
                                ...
                                className={cn(
                                    ...
                                    selectedTemplate?.id === template.id
                                        ? 'bg-brand-subtle text-primary border border-primary/25'
                                        : 'hover:bg-accent text-foreground-secondary border border-transparent'
                                )}
                            >
                                <div className="font-medium truncate">{template.name}</div>
                                <div className="text-xs text-foreground-muted mt-0.5 truncate">{template.subject || '(no subject)'}</div>
                            </button>
                        ))}
                    </div>
                    <div className="p-2 border-t border-border-subtle flex gap-1.5">
                        <button
                            onClick={handleNewTemplate}
                            className="flex-1 flex items-center justify-center gap-1.5 py-2 text-xs text-primary bg-brand-subtle rounded-lg hover:bg-brand-subtle"
                        >
                        ...
                        <button
                            ...
                            disabled={!selectedTemplate}
                            className="flex-1 flex items-center justify-center gap-1.5 py-2 text-xs text-foreground-secondary bg-muted rounded-lg hover:bg-accent disabled:opacity-40"
                        >
                ...
                <div className="flex-1 flex flex-col bg-card rounded-xl border border-border overflow-hidden min-w-0">
                    {selectedTemplate ? (
                        <>
                            {/* Toolbar */}
                            <div className="px-4 py-2.5 border-b border-border-subtle bg-muted/50 flex items-center gap-3 shrink-0">
                                {/* Subject */}
                                <div className="flex-1 flex items-center gap-2">
                                    <span className="text-xs font-medium text-foreground-secondary shrink-0">Subject:</span>
                                    <input
                                        value={subject}
                                        onChange={e => setSubject(e.target.value)}
                                        className="flex-1 text-sm border border-border rounded-md px-2 py-1 focus:outline-none focus:ring-2 focus:ring-primary-500 bg-card"
                                ...
                                <div className="flex gap-1">
                                    <button
                                        ...
                                        className="p-1.5 rounded hover:bg-accent text-foreground-secondary disabled:opacity-30"
                                        title="Undo"
                                    >
                                ...
                                {viewMode !== 'code' && (
                                    <div className="flex bg-card rounded-lg border border-border p-0.5">
                                        <button
                                            ...
                                            className={cn('p-1.5 rounded transition-all', deviceMode === 'desktop' ? 'bg-muted text-primary' : 'text-foreground-muted hover:text-foreground-secondary')}
                                            title="Desktop"
                                        >
                                        ...
                                        <button
                                            ...
                                            className={cn('p-1.5 rounded transition-all', deviceMode === 'mobile' ? 'bg-muted text-primary' : 'text-foreground-muted hover:text-foreground-secondary')}
                                            title="Mobile"
                                        >
                            ...
                            <div className="flex-1 flex min-h-0 overflow-hidden">
                                ...
                                {(viewMode === 'code' || viewMode === 'split') && (
                                    <div className={cn('flex flex-col', viewMode === 'split' ? 'w-1/2 border-r border-border' : 'w-full')}>
                                        <div className="px-3 py-1.5 bg-slate-900 flex items-center gap-2 shrink-0">
                                            ...
                                            <span className="ml-auto text-xs text-slate-300">{htmlContent.length} chars</span>
                                        </div>
                                        <textarea
                                            value={htmlContent}
                                ...
                                {(viewMode === 'preview' || viewMode === 'split') && (
                                    <div className={cn(
                                        ...
                                        <div className="px-3 py-1.5 bg-muted flex items-center gap-2 shrink-0">
                                            <Eye className="h-3.5 w-3.5 text-foreground-muted" />
                                            <span className="text-xs text-foreground-secondary font-medium">Live Preview</span>
                                            <span className="ml-auto text-xs text-foreground-muted flex items-center gap-1">
                                                <RefreshCw className="h-3 w-3" /> auto
                                            </span>
                                        </div>
                                        <div className="flex-1 bg-slate-200 flex items-start justify-center p-4 overflow-auto">
                                            <div className={cn(
                                                'bg-card shadow-xl transition-all duration-300 overflow-hidden',
                                                deviceMode === 'mobile'
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-foreground-muted">
                            ...
                            <button
                                onClick={handleNewTemplate}
                                className="mt-4 flex items-center gap-2 px-4 py-2 text-sm font-medium text-primary bg-brand-subtle rounded-lg hover:bg-brand-subtle"
                            >
