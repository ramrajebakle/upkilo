...
export default function EditLoyaltyRewardPage() {
    ...
    useEffect(() => {
        const fetchReward = async () => {
            ...
            setFetching(true);
            try {
                const res = await api.loyalty.getReward(id);
                reset(res.data);
            } catch (error) {
                console.error('Failed to fetch loyalty reward', error);
                toastError('Failed to load reward details');
                router.push('/loyalty');
            } finally {
                setFetching(false);
            }
    ...
    const onSubmit = async (data: LoyaltyFormData) => {
        setLoading(true);
        try {
            await api.loyalty.updateReward(id, data);
            toastSuccess('Loyalty reward updated successfully');
            router.push('/loyalty?updated=true');
        } catch (error) {
            console.error('Failed to update loyalty reward', error);
            toastError('Failed to update reward');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            router.push('/loyalty?deleted=true');
        } catch (error) {
