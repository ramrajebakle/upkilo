    ...
    return (
        <div className="p-6 max-w-5xl mx-auto space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 text-sm mt-0.5">{workflowName}</p>
                    </div>
                </div>
                <button onClick={fetchExecutions} className="p-2 rounded-lg hover:bg-slate-100 text-slate-500">
            ...
            <div className="grid grid-cols-4 gap-4">
                {[
                    { label: 'Total Runs', value: total, icon: <List className="h-4 w-4 text-slate-500" /> },
                    { label: 'Completed', value: stats.completed, icon: <CheckCircle className="h-4 w-4 text-emerald-500" /> },
                    { label: 'Failed', value: stats.failed, icon: <XCircle className="h-4 w-4 text-red-500" /> },
            ...
            {loading ? (
                ...
                <div className="text-center py-16 bg-white rounded-xl border border-slate-200">
                    ...
                    <p className="text-slate-500 text-sm mt-1">Run or trigger this workflow to see execution history</p>
                </div>
            ) : (
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                    {executions.map((exec, idx) => (
                        <div key={exec.id} className={idx < executions.length - 1 ? 'border-b border-slate-100' : ''}>
                            {/* Execution row */}
                            <div
                            ...
                            >
                                <div className="shrink-0">{statusIcon[exec.status] || <Clock className="h-4 w-4 text-slate-400" />}</div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2">
                                        <span className="font-mono text-xs text-slate-400">{exec.id.slice(0, 8)}...</span>
                                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColor[exec.status] || 'bg-slate-100 text-slate-600'}`}>
                                            {exec.status}
                                        </span>
                                        {exec.durationMs && (
                                    ...
                                    {exec.errorMessage && (
                                        <p className="text-xs text-red-500 mt-0.5 truncate">{exec.errorMessage}</p>
                                    )}
                                </div>
                                ...
                                <div className="shrink-0">
                                    {expandedId === exec.id ? <ChevronDown className="h-4 w-4 text-slate-400" /> : <ChevronRight className="h-4 w-4 text-slate-400" />}
                                </div>
                            </div>
                            ...
                            {expandedId === exec.id && (
                                <div className="bg-slate-50 border-t border-slate-100 px-5 py-4">
                                    {loadingLogs === exec.id ? (
                                        ...
                                        <p className="text-sm text-slate-400">No step logs available</p>
                                    ) : (
                                        <div className="space-y-2">
                                            <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Step-by-Step Execution</h4>
                                            {(logs[exec.id] || []).map((log, li) => (
                                                <div key={li} className="flex items-start gap-3 bg-white rounded-lg p-3 border border-slate-100">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-center gap-2 mb-0.5">
                                                            ...
                                                            <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${stepStatusColor[log.status] ? statusColor[log.status] || 'bg-slate-100 text-slate-600' : 'bg-slate-100 text-slate-600'}`}>
                                                                {log.status}
                                                            </span>
                                                            {log.durationMs && <span className="text-xs text-slate-400">{log.durationMs}ms</span>}
                                                        </div>
                                                        {log.message && <p className="text-xs text-slate-600">{log.message}</p>}
                                                        {log.errorDetails && (
