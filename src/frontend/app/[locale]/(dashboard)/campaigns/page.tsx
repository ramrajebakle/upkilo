...
export default function CampaignsPage() {
    ...
    const getStatusStyles = (status: string) => {
        switch (status) {
            ...
            case 'completed': return { bg: 'bg-muted', text: 'text-foreground-secondary', dot: 'bg-slate-400' };
            case 'draft': return { bg: 'bg-amber-50', text: 'text-amber-700', dot: 'bg-amber-500' };
            default: return { bg: 'bg-muted', text: 'text-foreground-secondary', dot: 'bg-slate-400' };
        }
    };
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div className="animate-fade-in-up">
                    ...
                    <p className="text-foreground-secondary">Create and manage email, SMS, and push campaigns</p>
                </div>
                <Link
                    href="/campaigns/new"
            ...
            {aggregate && (
                <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-6 text-white">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                        <div>
                            <p className="text-foreground-muted text-xs uppercase tracking-wide">Campaigns</p>
                            <p className="text-2xl font-bold mt-1">{aggregate.campaignCount}</p>
                        </div>
                        <div>
                            <p className="text-foreground-muted text-xs uppercase tracking-wide flex items-center gap-1"><Send className="h-3 w-3" /> Sent</p>
                            <p className="text-2xl font-bold mt-1">{aggregate.totalSent.toLocaleString()}</p>
                        </div>
                        <div>
                            <p className="text-foreground-muted text-xs uppercase tracking-wide flex items-center gap-1"><Eye className="h-3 w-3" /> Open Rate</p>
                            <p className="text-2xl font-bold mt-1 text-emerald-400">{aggregate.openRate}%</p>
                        </div>
                        <div>
                            <p className="text-foreground-muted text-xs uppercase tracking-wide flex items-center gap-1"><MousePointerClick className="h-3 w-3" /> Click Rate</p>
                            <p className="text-2xl font-bold mt-1 text-cyan-400">{aggregate.clickRate}%</p>
                        </div>
                        <div>
                            <p className="text-foreground-muted text-xs uppercase tracking-wide flex items-center gap-1"><DollarSign className="h-3 w-3" /> Revenue</p>
                ...
                <div className="space-y-4">
                    {autoResponders.length === 0 ? (
                        <div className="card-elevated py-16 text-center">
                            ...
                            <h3 className="text-lg font-semibold text-foreground mb-2">No auto-responders yet</h3>
                            <p className="text-foreground-secondary mb-4">Set up automated messages for events like new clients or completed bookings</p>
                        </div>
                    ) : (
                        autoResponders.map((ar) => (
                            <div key={ar.id} className="card-elevated p-5 flex items-center justify-between gap-4">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="font-semibold text-foreground">{ar.name}</p>
                                        <p className="text-sm text-foreground-secondary">
                                            Trigger: <span className="font-medium text-foreground">{triggerLabels[ar.triggerEvent] || ar.triggerEvent}</span>
                                            {ar.delayMinutes > 0 && (
                                ...
                                <span className={cn(
                                    ...
                                    ar.isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-muted text-foreground-secondary'
                                )}>
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <div className={cn(
                                ...
                                stat.color === 'violet' && 'bg-brand-subtle text-primary',
            ...
            <div className="space-y-4">
                {loading ? (
                    ...
                    <div className="card-elevated py-16 text-center animate-fade-in">
                        ...
                        <p className="text-foreground-secondary mb-6">Create your first campaign to engage with customers</p>
                            ...
                            >
                                <div className="p-6">
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-start justify-between gap-4 mb-3">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button className="p-2 rounded-lg text-foreground-muted hover:text-foreground-secondary hover:bg-accent transition-colors">
                                            ...
                                            {campaign.status !== 'draft' && (
                                                <div className="grid grid-cols-4 gap-4 pt-4 border-t border-border-subtle">
                                                    <div>
                                                        <p className="text-xs text-foreground-secondary mb-1">Sent</p>
                                                        <p className="font-semibold text-foreground">{campaign.sent.toLocaleString()}</p>
                                                    </div>
                                                    <div>
                                                        <p className="text-xs text-foreground-secondary mb-1">Opened</p>
                                                        <p className="font-semibold text-foreground">{campaign.opened.toLocaleString()}</p>
                                                    </div>
                                                    <div>
                                                        <p className="text-xs text-foreground-secondary mb-1">Open Rate</p>
                                                        <p className="font-semibold text-success-fg">{openRate}%</p>
                                                    </div>
                                                    <div>
                                                        <p className="text-xs text-foreground-secondary mb-1">Click Rate</p>
                                        ...
                                        {/* Arrow */}
                                        <ChevronRight className="h-5 w-5 text-slate-300 group-hover:text-primary group-hover:translate-x-1 transition-all self-center" />
                                    </div>
                                </div>
