...
function statusColor(status: string) {
  if (status === 'delivered' || status === 'success') return 'text-success-fg bg-emerald-500/10 border-emerald-500/20';
  if (status === 'failed') return 'text-danger-fg bg-red-500/10 border-red-500/20';
  if (status === 'pending') return 'text-warning-fg bg-amber-500/10 border-amber-500/20';
  return 'text-foreground-secondary bg-slate-500/10 border-slate-500/20';
}
...
export default function WebhooksIntegrationsPage() {
  ...
  if (loading && endpoints.length === 0) return (
      <div className="flex flex-col items-center justify-center min-h-[500px] gap-6">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.4em] text-foreground-secondary">Syncing Webhook Nodes...</p>
      </div>
  );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
        <div className="flex items-center gap-6">
            ...
            <div>
                ...
                <p className="text-[10px] font-black text-foreground-muted uppercase tracking-[0.3em] mt-1">Real-time Event Broadcasting Matrix</p>
            </div>
        </div>
        {!showForm && (
            <Button 
        ...
        {!showForm && endpoints.length > 0 && (
            ...
            >
                {[
                    ...
                    { label: 'Active Channels', value: endpoints.filter(e => e.isActive).length, icon: Zap, color: 'text-success-fg', bg: 'bg-emerald-500/10' },
        ...
        {showForm && (
            ...
            >
                <div className="flex items-center justify-between relative z-10">
                    ...
                    <button onClick={() => setShowForm(false)} className="h-10 w-10 flex items-center justify-center rounded-xl bg-slate-50 dark:bg-slate-950 text-foreground-muted hover:text-slate-900 transition-colors">✕</button>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-10 relative z-10">
                    <div className="space-y-4">
                        <label className="text-[10px] font-black text-foreground-muted uppercase tracking-widest ml-1">Endpoint Alias</label>
                        <Input
                    ...
                    <div className="space-y-4">
                        <label className="text-[10px] font-black text-foreground-muted uppercase tracking-widest ml-1">Secure Destination URL</label>
                        <Input
                ...
                <div className="space-y-8 relative z-10 pt-4">
                    <div className="flex items-center justify-between">
                        <label className="text-[10px] font-black text-foreground-muted uppercase tracking-widest ml-1">Event Subscription Matrix</label>
                        <button
                    ...
                    <div className="space-y-8 max-h-[400px] overflow-y-auto pr-4 scrollbar-premium">
                        {EVENT_CATEGORIES.map(cat => (
                            <div key={cat.category} className="space-y-4">
                                <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.3em] border-b border-slate-50 dark:border-slate-850 pb-2">{cat.category} PROTOCOLS</p>
                                        ...
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                form.events.includes(event)
                                                    ...
                                                    : 'bg-white dark:bg-slate-950 text-foreground-muted border border-slate-100 dark:border-slate-850 hover:border-primary-500/40'
                                            )}
                                        >
                                            {event}
                                        </button>
                ...
                <div className="flex gap-6 pt-10 border-t border-slate-50 dark:border-slate-850 relative z-10">
                    ...
                    <Button variant="outline" onClick={() => setShowForm(false)} className="h-16 px-12 rounded-[24px] font-black uppercase tracking-widest text-[10px] text-foreground-muted">Abort</Button>
                </div>
      ...
      <div className="space-y-8">
        {endpoints.length === 0 && !showForm ? (
            <div className="p-20 bg-white dark:bg-slate-900 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-2xl shadow-slate-200/40 dark:shadow-none text-center">
                ...
                <p className="text-[10px] font-black text-foreground-muted uppercase tracking-widest mt-2 mb-10">No active real-time integration conduits found in the matrix.</p>
                            ...
                            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-10">
                                <div className="flex-1 min-w-0 space-y-6">
                                        ...
                                        <div>
                                            ...
                                            <span className={cn(
                                                ...
                                                endpoint.isActive 
                                                    ...
                                                    : 'bg-slate-100 dark:bg-slate-800 text-foreground-muted border-slate-200 dark:border-slate-700'
                                            )}>
                                    ...
                                    <div className="flex items-center gap-4 bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-transparent dark:border-slate-850 shadow-inner group-hover:bg-white dark:group-hover:bg-slate-900 transition-colors">
                                        <code className="text-[10px] font-black text-foreground-muted tracking-tighter truncate max-w-lg uppercase">{endpoint.url}</code>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {(endpoint.events || []).slice(0, 10).map(event => (
                                            <span key={event} className="px-3 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest bg-slate-100 dark:bg-slate-850 text-foreground-secondary border border-transparent dark:border-slate-800">{event}</span>
                                        ))}
                                    ...
                                    {endpoint.secret && (
                                        <div className="flex items-center gap-4 p-4 bg-slate-50/50 dark:bg-slate-950/20 rounded-2xl border border-transparent dark:border-slate-850">
                                            ...
                                            <span className="text-[9px] font-black text-foreground-muted uppercase tracking-widest">Secret Hash:</span>
                                            ...
                                            <div className="flex gap-2 ml-auto">
                                                <button onClick={() => setShowSecret(showSecret === endpoint.id ? null : endpoint.id)} className="p-2 text-foreground-muted hover:text-primary-500 transition-colors">
                                                    {showSecret === endpoint.id ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                                                </button>
                                                <button onClick={() => copyToClipboard(endpoint.secret!)} className="p-2 text-foreground-muted hover:text-primary-500 transition-colors">
                                                    <Copy className="h-4 w-4" />
                                                </button>
                                            </div>
                                        </div>
                                    ...
                                    {endpoint.deliveryCount !== undefined && (
                                        <div className="flex flex-wrap items-center gap-8 pt-4 border-t border-slate-50 dark:border-slate-850">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-[10px] font-black text-foreground-muted uppercase tracking-widest tabular-nums">{endpoint.deliveryCount} DELIVERIES</span>
                                            </div>
                                            {endpoint.successCount !== undefined && (
                                                <div className="flex items-center gap-3">
                                                    <div className="w-2 h-2 rounded-full bg-emerald-500" />
                                                    <span className="text-[10px] font-black text-success-fg uppercase tracking-widest tabular-nums">{endpoint.successCount} SYNCED</span>
                                                </div>
                                            )}
                                            {endpoint.failureCount !== undefined && endpoint.failureCount > 0 && (
                                                <div className="flex items-center gap-3">
                                                    <div className="w-2 h-2 rounded-full bg-red-500" />
                                                    <span className="text-[10px] font-black text-danger-fg uppercase tracking-widest tabular-nums">{endpoint.failureCount} DROPPED</span>
                                                </div>
                                            )}
                                        </div>
                                    )}
                            ...
                            {showDeliveries === endpoint.id && (
                                ...
                                >
                                    <div className="flex items-center justify-between mb-8">
                                        ...
                                        <button onClick={() => fetchDeliveries(endpoint.id)} className="p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-foreground-muted hover:text-primary-500 transition-all">
                                    ...
                                    {deliveries.filter(d => d.webhookEndpointId === endpoint.id).length === 0 ? (
                                        <div className="text-center py-20 bg-white/50 dark:bg-slate-900/50 rounded-[32px] border border-dashed border-slate-200 dark:border-slate-800">
                                            <p className="text-[10px] font-black text-foreground-muted uppercase tracking-widest">Atmosphere clear. No packet collisions detected.</p>
                                        </div>
                                    ) : (
                                                                ...
                                                                <div className="flex items-center gap-4">
                                                                    {delivery.responseStatusCode && (
                                                                        <span className={cn(
                                                                            ...
                                                                            delivery.responseStatusCode < 300 ? 'text-success-fg' : 'text-danger-fg'
                                                                        )}>
                                                                            HTTP {delivery.responseStatusCode}
                                                                        </span>
                                                                    )}
                                                                    <span className="text-[8px] font-black text-foreground-muted uppercase tracking-widest">Attempt {delivery.attemptCount} :: {new Date(delivery.createdAt).toLocaleTimeString()}</span>
                                                                </div>
                                                            </div>
                                                        </div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
            ...
            <div className="flex-1 space-y-4">
                ...
                <div className="flex flex-wrap gap-4 pt-4">
                    <Button variant="secondary" className="h-12 px-8 rounded-xl bg-slate-800 border-slate-700 text-primary-400 hover:bg-slate-700 font-black uppercase tracking-widest text-[9px]">
                        <Terminal className="h-4 w-4 mr-2" /> Source Node Example
                    </Button>
                    <Button variant="secondary" className="h-12 px-8 rounded-xl bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-700 font-black uppercase tracking-widest text-[9px]">
