...
export default function PaymentsPage() {
    ...
    if (loading && payments.length === 0) return (
        <div className="flex flex-col items-center justify-center min-h-[500px] gap-6">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-foreground-secondary">Syncing Financial Matrix...</p>
        </div>
    );
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-12 animate-fade-in pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-black text-foreground-muted uppercase tracking-[0.3em] mt-1">Global Revenue Tracking and Capitalization Corridor</p>
                    </div>
                </div>
                <div className="flex items-center gap-4">
                    ...
                    <Link href="/payments/split">
                        <Button className="h-14 px-8 rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-2xl shadow-emerald-500/20 active:scale-95 transition-all flex items-center gap-3 bg-emerald-600 hover:bg-emerald-700 text-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {[
                    { label: 'Cumulative Revenue', value: formatCurrency(totalRevenue), icon: DollarSign, color: 'text-success-fg', bg: 'bg-emerald-500/10', trend: '+14.2%' },
                    { label: 'Awaiting Uplink', value: formatCurrency(pendingAmount), icon: Clock, color: 'text-warning-fg', bg: 'bg-amber-500/10', trend: 'Audit Req' },
                    ...
                    {['all', 'completed', 'pending', 'refunded', 'failed'].map((status) => (
                        <button
                            ...
                            className={cn(
                                ...
                                statusFilter === status
                                    ? 'bg-card text-success-fg shadow-[var(--shadow-card)] scale-105 active:scale-95'
            ...
            <div className="max-w-full min-w-0 bg-white dark:bg-slate-900 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-2xl shadow-slate-200/40 dark:shadow-none overflow-hidden xl:min-h-[500px]">
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-slate-50 dark:border-slate-850 bg-slate-50/50 dark:bg-slate-950/50">
                                <th className="px-10 py-6 text-[10px] font-black text-foreground-muted uppercase tracking-[0.3em]">Originator / Hash</th>
                                <th className="px-10 py-6 text-[10px] font-black text-foreground-muted uppercase tracking-[0.3em]">Service Vector</th>
                                <th className="px-10 py-6 text-[10px] font-black text-foreground-muted uppercase tracking-[0.3em]">Capitalization</th>
                                <th className="px-10 py-6 text-[10px] font-black text-foreground-muted uppercase tracking-[0.3em]">Protocol</th>
                                <th className="px-10 py-6 text-[10px] font-black text-foreground-muted uppercase tracking-[0.3em]">Status</th>
                                <th className="px-10 py-6 text-right text-[10px] font-black text-foreground-muted uppercase tracking-[0.3em]">Audit</th>
                            </tr>
                        </thead>
                                    ...
                                    <tr key={payment.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-950/20 transition-all duration-300">
                                        <td className="px-10 py-6">
                                            <div className="flex items-center gap-6">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[9px] font-black text-foreground-muted mt-0.5 tracking-tighter uppercase font-mono">{payment.transactionId || payment.id.substring(0, 12)}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-10 py-6">
                                        ...
                                        <td className="px-10 py-6">
                                            <div className="flex items-center gap-3 px-3 py-1.5 bg-slate-50 dark:bg-slate-950 rounded-xl border border-transparent dark:border-slate-850 w-fit">
                                                <MethodIcon className="h-3.5 w-3.5 text-foreground-muted" />
                                                <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest">{payment.method || 'Unknown'}</span>
                                            </div>
                                        </td>
                                        <td className="px-10 py-6">
                                            <span className={cn(
                                        ...
                                        <td className="px-10 py-6 text-right">
                                            <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-4 group-hover:translate-x-0">
                                                <Link href={`/payments/invoices/${payment.id}`} className="p-3 hover:bg-white dark:hover:bg-slate-800 rounded-xl border border-transparent hover:border-slate-100 dark:hover:border-slate-700 shadow-sm transition-all">
                                                    <Eye className="h-4 w-4 text-foreground-muted hover:text-primary-500" />
                                                </Link>
                ...
                {filteredPayments.length === 0 && (
                    <div className="p-20 text-center">
                        ...
                        <p className="text-[10px] font-black text-foreground-muted uppercase tracking-widest mt-2">The financial vault is currently at zero density.</p>
                    </div>
                )}
            </div>
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
                    ...
                    <div className="flex-1 space-y-4">
                        ...
                        <div className="flex flex-wrap gap-4 pt-4">
                            <Button variant="secondary" className="h-12 px-8 rounded-xl bg-slate-800 border-slate-700 text-emerald-400 hover:bg-slate-700 font-black uppercase tracking-widest text-[9px]">
