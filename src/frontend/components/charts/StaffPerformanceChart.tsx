...
function CustomTooltip({ active, payload }: any) {
    ...
    return (
        <div className="bg-popover text-popover-foreground border border-border shadow-[var(--shadow-popover)] rounded-xl px-4 py-3 shadow-2xl text-xs space-y-1">
...
export function StaffPerformanceChart({ data, height = 240, variant = 'bar' }: StaffPerformanceChartProps) {
    if (!data || data.length === 0) {
        return (
            <div className="flex items-center justify-center h-[240px] text-foreground-muted text-sm">
                No data available
            </div>
        );
    }
                    ...
                    <div key={staff.name} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex justify-between text-xs mb-1">
                                ...
                                <span className="text-foreground-secondary ml-2 flex-shrink-0">
                        ...
                        {staff.rating && (
                            <span className="text-xs font-semibold text-warning-fg flex-shrink-0">
