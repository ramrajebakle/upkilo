    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden group">
      <div className="relative h-48 w-full bg-gray-100 overflow-hidden">
        ...
        <div className="absolute top-4 left-4 flex gap-2">
           ...
           {listing.isVerified && (
             <span className="px-3 py-1 bg-white/90 backdrop-blur-sm text-primary-700 text-[10px] font-black rounded-full shadow-lg flex items-center gap-1 uppercase tracking-wider border border-white">
      ...
      <div className="p-6">
        <div className="flex items-start justify-between mb-2">
          <div>
            <h3 className="text-lg font-bold text-gray-900 group-hover:text-primary-600 transition-colors">{listing.businessName}</h3>
            <p className="text-xs font-medium text-gray-500 flex items-center gap-1 mt-0.5">
              <span className="text-primary-500">{listing.category}</span>
              ...
              <span className="flex items-center gap-1 bg-amber-50 text-amber-700 px-1.5 py-0.5 rounded leading-none text-[10px] font-bold">
                <Star className="h-2.5 w-2.5 fill-amber-500 text-amber-500" /> {listing.averageRating || 5.0} ({listing.reviewCount || 0})
              </span>
            </p>
          </div>
        </div>

        <p className="text-sm text-gray-600 line-clamp-2 min-h-[40px] mt-3">
